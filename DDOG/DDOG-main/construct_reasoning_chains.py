    ...
    for example in tqdm(data, desc="Generating reasoning chains", total=len(data)):
        ...
        for ctx in example["ctxs"]:
            for triple_item in ctx["triples"]:
                triples.append("<{}; {}; {}>".format(triple_item["head"], triple_item["relation"], triple_item["tail"]))#三元组字符串
...
def construct_reasoning_chains_from_subgraph_and_dynamic(
    args,
    question: str,
    initial_triples: List[Tuple[str, str, str]],
    dynamic_triples: List[Tuple[str, str, str]],
    max_chain_length: int = 4,
    num_chains: int = 20
) -> List[Dict]:
    """
    基于子图初始三元组和动态检索三元组构造推理链
    Args:
        args: 参数配置
        question: 问题
        initial_triples: 来自子图的初始三元组
        dynamic_triples: 来自动态检索的三元组
        max_chain_length: 最大推理链长度
        num_chains: 生成的推理链数量
    Returns:
        List[Dict]: 推理链列表
    """
    ...
    for i, (head, relation, tail) in enumerate(initial_triples):
        all_triples.append(f"<{head}; {relation}; {tail}>")
        triple_positions.append(f"subgraph_{i}")
    
    
    for i, (head, relation, tail) in enumerate(dynamic_triples):
        all_triples.append(f"<{head}; {relation}; {tail}>")
        triple_positions.append(f"dynamic_{i}")
    
    num_total_triples = len(all_triples)
    
    if num_total_triples == 0:
        logger.warning("没有可用的三元组来构造推理链")
        return []
    
    
    args.ranking_model == "e5_mistral":
    triples_embeddings = get_e5_mistral_embeddings_for_document(all_triples, max_length=128, batch_size=2)
    ...
    for j in range(max_chain_length):
        ...
        queries = [  
            "knowledge triples: {}\nquestion: {}".format(
                " ".join([all_triples[idx] for idx in path]), question
            )
            for path in paths
        ]
        ...
        topk_most_relevant_triples_indices = torch.topk(
            ...
            k=min(args.num_choices, num_total_triples), 
            dim=1
        )[1].tolist()
        ...
        prompts = get_llama3_generate_reasoning_chains_prompts_chat_format(
            ...
            hop=j, 
            question=question,
            existing_triples=[[all_triples[idx] for idx in path] for path in paths],
            candidate_triples=[
                [all_triples[idx] for idx in candidate_triples_indices] \
                    for candidate_triples_indices in topk_most_relevant_triples_indices
            ]
        )
        ...
        generated_token_ids, generated_token_logits = model_generate(
            inputs, 
        ...
        for i in range(len(paths)):
            ...
            if torch.all(torch.isnan(topk_choices_probs[i])):
                logger.warning("No choice in generated results!")
                new_paths.append(paths[i])
                new_paths_scores.append(paths_scores[i])
                new_paths_finished.append(False)
                continue 
    ...
    reasoning_chains = [
        {
            "triples": [
                {
                    "triple": all_triples[triple_index], 
                    "triple_position": triple_positions[triple_index],
                    "source": "subgraph" if triple_positions[triple_index].startswith("subgraph") else "dynamic"
                } 
                for triple_index in path if triple_index >= 0 
            ],
            "score": path_score,
            "length": len([idx for idx in path if idx >= 0])
        }
        for path, path_score in zip(paths, paths_scores)
    ]
    
    logger.info(f"成功构造 {len(reasoning_chains)} 条推理链")
