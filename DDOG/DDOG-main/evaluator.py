...
def evaluate(args, tokenizer, dataloader, model):
    ...
    bleu_scores_list, rouge_l_scores_list = [], [] 
    ...
    for batch_index, batch_inputs in tqdm(dataloader, desc="Evaluation", total=len(dataloader)):
        ...
        for i, o in enumerate(batch_generated_token_ids):
            ...
            bleu_score = calculate_bleu_score(ans, gold[0])
            rouge_l_score = calculate_rouge_l_score(ans, gold[0])
            bleu_scores_list.append(bleu_score)
            rouge_l_scores_list.append(rouge_l_score)
    ...
    metrics["bleu"] = np.mean(bleu_scores_list)  
    metrics["rouge_l"] = np.mean(rouge_l_scores_list)  
...
def calculate_rouge_l_score(predicted: str, reference: str) -> float:
