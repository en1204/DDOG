...
from utils.graph_utils import build_graph_from_triples, entity_linking
from utils.ercr_utils import ERCRChecker
from dynamic_retriever import DynamicRetriever
from construct_reasoning_chains import construct_reasoning_chains_from_subgraph_and_dynamic
...
from utils.graph_utils import build_graph_from_triples
from utils.ercr_utils import ERCRChecker
from dynamic_retriever import DynamicRetriever
from construct_reasoning_chains import construct_reasoning_chains_from_subgraph_and_dynamic
...
def setup_parser():
    ...
    parser.add_argument("--save_graph_file", type=str, default="data/hotpotqa/global_kg.pkl")
    parser.add_argument("--entity_linking", action="store_true", help="Enable entity linking between documents")
...
def get_dataset_demonstrations(dataset):
    if dataset == "hotpotqa":
        ...
        from prompts import generate_knowledge_triples_wikihop_examplars
        demonstrations = generate_knowledge_triples_wikihop_examplars
    elif dataset == "cwq":
        from prompts import generate_knowledge_triples_cwq_examplars
        demonstrations = generate_knowledge_triples_cwq_examplars
    else:
...
def parse_model_output(triples_text: str) -> List[Tuple[str, str, str]]:
    """Enhanced triple parsing with entity normalization"""
    results = []
    for one_triple_text in re.findall(r'<([^>]*)>', triples_text):
        ...
        head = normalize_entity(head.strip())
        relation = normalize_relation(relation.strip())
        tail = normalize_entity(tail.strip())
...
def build_global_knowledge_graph(triples_list: List[List[Dict]], args) -> nx.DiGraph:
    """构建全局知识图谱"""
    global_graph = nx.DiGraph()
    
    all_triples = []
    for doc_triples in triples_list:
    ...
    global_graph = build_graph_from_triples(all_triples)
    

    if args.entity_linking:
        global_graph = entity_linking(global_graph)
...
def generate_triples_for_document_list(args, document_list: List[Dict[str, Optional[Union[str, List[str], List[int]]]]], verbose=False) -> List[List[Dict[str, Union[str, list]]]]:
    ...
    for j, (document, generated_content) in enumerate(zip(document_list, generated_contents)):
        triples = parse_model_output(generated_content)
...
def add_sentence_index_to_generated_triples(document_list: List[Dict[str, Optional[Union[str, List[str], List[int]]]]], triples_list: List[List[Dict[str, str]]]) -> List[List[dict]]:
...
def generate_document_knowledge_graph(args):
    ...
    if os.path.exists(save_data_file):
        print(f"{save_data_file} already exists, loading cached file...")
        data = load_json(save_data_file)
    else:
        ...
        for example in tqdm(data, desc="Obtaining Unique documents", total=len(data)):
            for ctx in example["ctxs"]:
                ...
                if document_hash_id in id2document:
                    assert ctx["title"] == id2document[document_hash_id]["title"]
                    continue
        ...
        for example in tqdm(data, desc="Updating document triples", total=len(data)):
            for i, ctx in enumerate(example["ctxs"]):
                document_hash_id = hash_object(ctx["title"] + " " + ctx["text"])
                document_triples = document_triples_with_sentence_index_list[document_id_to_index[document_hash_id]]
    ...
    for example in data:
        for ctx in example["ctxs"]:
            all_triples.extend(ctx["triples"])
    
    global_graph = build_global_knowledge_graph(all_triples, args)
    
    print(f"Saving global knowledge graph to {args.save_graph_file}")
    with open(args.save_graph_file, 'wb') as f:
        pickle.dump(global_graph, f)

def generate_and_process_knowledge_graph(args):
    """
    生成知识图谱并执行完整的处理流程
    """
    generate_document_knowledge_graph(args)
    
    print(f"Loading global knowledge graph from {args.save_graph_file}")
    with open(args.save_graph_file, 'rb') as f:
        global_graph = pickle.load(f)
    
    print(f"全局知识图谱包含 {global_graph.number_of_nodes()} 个节点和 {global_graph.number_of_edges()} 条边")
    ...
    for query in sample_queries:
        print(f"\n处理查询: {query}")
        try:
            result = process_knowledge_graph_with_ercr_and_dynamic_retrieval(
                args, global_graph, query
            )
            print(f"查询处理完成: {result.keys()}")
...
if __name__ == "__main__":
    args = setup_parser()
    generate_and_process_knowledge_graph(args) 